...
class XLRScheduler(ALRScheduler):
    def __init__(self, lr_base: float, optimizer: Optimizer, method: str, step: int= None, decay_rate: float = None,
                 step2: int= None, total_step :int = None, min_lr: float = 1e-5) -> None:
        ...
        """
        assert method in ["decay","warmup"], "Learning rate schdedule method can only be 'decay' or warmup"
    ...
    def decay(self,step :int) -> float:
        ...
        if step <= self._step:
            ...
            lr = self._lr_base * lr_factor if self._lr_base * lr_factor > self._min_lr else self._min_lr
        else:
    ...
    def warmup(self, step: int) -> float:
        assert self._step2 is not None and self._total_step is not None, \
        ...
        if step <= self._step:
            ...
            lr_factor = max(1, float(self._total_step - step)) / float(max(1.0, self._total_step- self._step2))
            lr = self._lr_base * lr_factor if self._lr_base*lr_factor > self._min_lr else self._min_lr
        else:
            lr = self._min_lr
        return lr

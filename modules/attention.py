...
class SelfAttention(nn.Module):
    ...
    def __init__(self,
        ...
        if self.use_ffn:
            ...
            self.ffn = FFN(in_size = hidden_size,
                           mid_size = mid_size,
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, v, k, q, mask=None):
        ...
        v = self.linear_v(v).view(batch_size,
                                  -1,
                                  self.multi_head,
        ...
        k = self.linear_k(k).view(batch_size,
                                  -1,
                                  self.multi_head,
                                  self.hidden_size_head).transpose(1,2)

        q = self.linear_q(q).view(batch_size,
                                  -1,
                                  self.multi_head,
                                  self.hidden_size_head).transpose(1,2)
...
class CrossAttention(nn.Module):
    ...
    def __init__(self,
        ...
        if self.use_ffn:
            assert mid_size is not None, "Mid size should not be None"
            assert act_fun is not None, "Activigation function should not be None"
            self.ffn = FFN(in_size = hidden_size,
                       mid_size = mid_size,
                       out_size = hidden_size,
                       dropout_r=dropout_r,
